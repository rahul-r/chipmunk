...
macro_rules! wait_for_db {
    ($x:expr) => {
        ...
        println!();
        // Wait for the database to complete its tasks
        sleep(Duration::from_secs(10)).await; // TODO: Find a better way to wait for the database instead of hardcoding delays
    };
}
